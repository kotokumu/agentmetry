...
4. macOS imports a Developer ID Application certificate into an ephemeral
   keychain, then signs, notarizes, staples, and verifies the app and DMG.
5. Package files are uploaded as short-lived workflow artifacts and to a draft
   GitHub Release.
6. The publish job verifies the complete asset set and publishes the draft only
