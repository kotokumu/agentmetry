...
`.github/workflows/release.yml` is the only distribution workflow. It has no
pull-request trigger. A `v*` tag is the publication contract:

1. Native macOS, Windows, and Linux runners install the locked dependencies.
2. Each runner invokes the same root package entrypoint used locally.
3. The tag version is checked against `src-tauri/tauri.conf.json`.
4. Package files are uploaded as short-lived workflow artifacts.
5. The publish job attaches those files to a GitHub Release for the tag.

The workflow currently publishes unsigned packages. Signing, notarization,
checksums, and update metadata are later release stages and must not be added
until their credentials and security gates are available.
