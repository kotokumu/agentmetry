...
export class TokenChart extends LocalizedElement {
  ...
  static styles = css`
    ...
    .total span { color: var(--am-muted); font-size: .75rem; text-transform: uppercase; letter-spacing: .08em; }
    .total strong { color: var(--am-accent); font: 700 1.05rem/1.2 "SFMono-Regular", "Cascadia Code", monospace; }
    .group { margin-top: 3px; color: var(--am-muted); font-size: .75rem; text-transform: uppercase; letter-spacing: .08em; }
    .row { display: grid; grid-template-columns: 92px 1fr 72px; align-items: center; gap: 10px; }
    .label, .value { font: 0.75rem/1.2 "SFMono-Regular", "Cascadia Code", monospace; }
    .label { color: var(--am-muted); }
    .label.modifier { padding-left: 12px; }
    .value { text-align: right; color: var(--am-text); }
    .track { height: 7px; border-radius: 99px; background: var(--am-track); overflow: hidden; }
