...
export class AgentTree extends LitElement {
  ...
  render() {
    ...
    return html`<div class="viewport" role="tree" aria-label="Agent delegation topology"><div class="graph" style=${`width:${layout.width}px;height:${layout.height}px`}>
      ...
      ${layout.nodes.map((node) => renderGraphNode(
        node,
        this.selectedAgentId,
        (event: Event) => this.selectAgent(event, node.node.agent.agentId),
        (event: CustomEvent<{ open: boolean }>) => this.tokenBreakdownToggled(event),
      ))}
    </div></div>`;
  }

  protected updated() {
    const nodes = [...(this.shadowRoot?.querySelectorAll<HTMLElement>(".node") ?? [])];
    this.syncNodeHeights(nodes);
    if (typeof ResizeObserver === "undefined") return;
    this.nodeResizeObserver ??= new ResizeObserver(() => {
      this.syncNodeHeights([...(this.shadowRoot?.querySelectorAll<HTMLElement>(".node") ?? [])]);
    });
    this.nodeResizeObserver.disconnect();
    nodes.forEach((node) => this.nodeResizeObserver?.observe(node));
  }
  ...
  private tokenBreakdownToggled(event: CustomEvent<{ open: boolean }>) {
    const node = (event.currentTarget as HTMLElement).closest<HTMLElement>(".node");
    if (node) this.syncNodeHeights([...(this.shadowRoot?.querySelectorAll<HTMLElement>(".node") ?? [])]);
  }

  private syncNodeHeights(nodes: readonly HTMLElement[]) {
    const nodeHeights = new Map<string, number>();
    for (const node of nodes) {
      const agentId = node.dataset.agentId;
      if (!agentId) continue;
      const measuredHeight = node.offsetHeight;
      if (measuredHeight > 0) nodeHeights.set(agentId, Math.max(NODE_MIN_HEIGHT, measuredHeight));
      else if (this.nodeHeights.has(agentId)) nodeHeights.set(agentId, this.nodeHeights.get(agentId)!);
    }
    if (sameNodeHeights(this.nodeHeights, nodeHeights)) return;
    this.nodeHeights = nodeHeights;
  }
}

export type AgentNode = Readonly<{ agent: AgentSession; children: readonly AgentNode[] }>;
export type LayoutNode = Readonly<{ node: AgentNode; parentID?: string; depth: number; centerX: number; centerY: number; height: number }>;
export type TreeLayout = Readonly<{ nodes: readonly LayoutNode[]; width: number; height: number }>;

const NODE_WIDTH = 190;
const NODE_MIN_HEIGHT = 116;
const LEVEL_GAP = 62;
const ROW_GAP = 220;
...
const renderConnector = (parent: LayoutNode, child: LayoutNode): unknown => {
  ...
  const childTop = child.centerY - child.height / 2;
...
export const layoutAgentTree = (roots: readonly AgentNode[], nodeHeights: ReadonlyMap<string, number> = new Map()): TreeLayout => {
  const groups = [...roots]
    ...
    .map((root) => {
      ...
      visit(root, 0);
      const levelHeights = levels.map((level) => Math.max(...level.map(({ node }) => nodeHeights.get(node.agent.agentId) ?? NODE_MIN_HEIGHT)));
      const maxLevelWidth = Math.max(NODE_WIDTH, ...levels.map((level) => NODE_WIDTH + Math.max(0, level.length - 1) * ROW_GAP));
      const height = levelHeights.reduce((total, levelHeight) => total + levelHeight, 0) + Math.max(0, levels.length - 1) * LEVEL_GAP + 24;
      return { levels, levelHeights, height, maxLevelWidth, width: maxLevelWidth + SIDE_PADDING * 2 };
    });
  ...
  groups.forEach((group, groupIndex) => {
    ...
    group.levels.forEach((level, levelIndex) => {
      ...
      const levelHeight = group.levelHeights[levelIndex];
      level.forEach((entry, index) => {
        const column = index;
        const height = nodeHeights.get(entry.node.agent.agentId) ?? NODE_MIN_HEIGHT;
        nodes.push({
          ...entry,
...
const renderGraphNode = (
  ...
  tokenBreakdownToggled: (event: CustomEvent<{ open: boolean }>) => void,
): unknown => html`<div
  class="node"
  role="treeitem"
  ...
  tabindex="0"
  style=${`left:${layout.centerX - NODE_WIDTH / 2}px;top:${layout.centerY - layout.height / 2}px;width:${NODE_WIDTH}px`}
  @click=${select}
...
const sameNodeHeights = (left: ReadonlyMap<string, number>, right: ReadonlyMap<string, number>) =>
  left.size === right.size && [...left].every(([agentId, height]) => right.get(agentId) === height);
