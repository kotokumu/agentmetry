...
export class TokenBreakdown extends LitElement {
  ...
  static styles = css`
    ...
    .grid { display: grid; grid-template-columns: minmax(90px, 1fr) auto; gap: 4px 12px; margin-top: 7px; padding: 8px 10px; border-left: 2px solid var(--am-accent); background: var(--am-surface-strong); }
  ...
  render() {
    ...
    return html`<div class="summary" aria-label="Token usage">
      <strong class="total">${this.usage.total === null ? hasUsage ? "Partial" : "N/A" : this.usage.total.toLocaleString()}</strong>
