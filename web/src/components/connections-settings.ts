...
export class ConnectionsSettings extends LocalizedElement {
  ...
  static styles = css`
    ...
    .category { display: grid; gap: 12px; }
    .category + .category { border-top: 1px solid var(--am-border); padding-top: 24px; }
    .category > h2 { font-size: 1.15rem; letter-spacing: -.01em; }
    .source-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; align-items: start; }
    .general-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 28rem), 1fr)); gap: 16px; align-items: start; }
    article, .panel, .controls { border: 1px solid var(--am-border); border-radius: 10px; padding: 18px; background: var(--am-surface-raised); }
    h2, h3, p { margin: 0; }
    ...
    @media (max-width: 700px) { .source-grid, .general-grid, .note-grid { grid-template-columns: 1fr; } }
  `;

  render() {
    return html`<div class="stack">
      <section class="category" data-settings-category="general">
        <h2>${msg("General", { id: "settingsCompletion.generalHeading" })}</h2>
      ...
      <section class="category" data-settings-category="telemetry-ingestion">
        <h2>${msg("Telemetry ingestion", { id: "settingsCompletion.telemetryHeading" })}</h2>
        <div class="panel"><h3>${msg("Send telemetry to Agentmetry", { id: "settingsCompletion.sourcesHeading" })}</h3><p class="lead">${msg("Copy these settings into each source configuration, then restart it to send telemetry to Agentmetry. Agentmetry does not edit source files.", { id: "settingsCompletion.sourcesIntro" })}</p></div>
        ...
        <div class="panel"><h3>${msg("OTLP receiver endpoints", { id: "settingsCompletion.endpointsHeading" })}</h3><p class="lead">${msg("Defaults are OTLP gRPC 4317 and OTLP HTTP 4318. If you start Agentmetry with custom receiver ports, replace the port in the source configuration. The Web UI, HTTP API, and MCP use 17890; that browser address is not an OTLP receiver.", { id: "settingsCompletion.endpointsBody" })}</p><p class="copy-status" aria-live="polite">${this.copyError ? msg("Copy failed — select the text manually", { id: "settingsCompletion.copyFailedMessage" }) : this.copied ? msg("Copied to clipboard", { id: "settingsCompletion.copied" }) : ""}</p></div>
        <div class="note-grid">
          <div class="panel"><h3>${msg("Capture scope", { id: "settingsCompletion.scopeHeading" })}</h3><p class="lead">${msg("Agentmetry retains accepted telemetry locally. What is available depends on what the source reports and which content settings are enabled.", { id: "settingsCompletion.scopeIntro" })}</p></div>
          <div class="panel"><h3>${msg("Reported and retained data", { id: "settingsCompletion.retentionHeading" })}</h3><p class="lead">${msg("Prompts, responses, tool details, and file contents may be unreported, redacted, or unavailable. Missing values are shown as unreported; the app does not infer complete conversations or outcomes.", { id: "settingsCompletion.retentionBody" })}</p></div>
        </div>
      </section>
      <section class="category" data-settings-category="data-management">
        <h2>${msg("Data management", { id: "settingsCompletion.dataHeading" })}</h2>
        <am-retention-settings></am-retention-settings>
      </section>
      <section class="category" data-settings-category="mcp-access">
        <h2>${msg("MCP access", { id: "settingsCompletion.mcpHeading" })}</h2>
        <div class="panel"><p>${msg("Use the local MCP URL in a client to query telemetry. MCP is stateless and read only. The Web UI origin and OTLP receiver endpoint are separate addresses.", { id: "settingsCompletion.mcpIntro" })}</p><div class="control"><am-mcp-connection inline></am-mcp-connection></div></div>
      </section>
    </div>`;
  }
