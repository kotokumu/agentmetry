...
FROM spans JOIN requested USING (trace_id, span_id)`, string(payload))
	...
	affectedSessions := make(map[sessionKey]struct{})
	for _, span := range batch.Spans {
		if canonical.IsSemanticSpan(span) && span.Agent.RunID != "" {
			affectedSessions[sessionKey{source: normalizeSource(span.Source), runID: span.Agent.RunID}] = struct{}{}
		}
	}
	for _, log := range batch.Logs {
		if log.Agent.RunID != "" {
			affectedSessions[sessionKey{source: normalizeSource(log.Source), runID: log.Agent.RunID}] = struct{}{}
		}
	}
	for _, scope := range previousSpans {
		addActivityTargets(targetSet, scope.source, scope.session, scope.trace)
		if scope.session != "" {
			key := sessionKey{source: scope.source, runID: scope.session}
...
func addPublishedSessionTargets(ctx context.Context, transaction *sql.Tx, targets *query.ChangeTargetSet, sessions map[sessionKey]struct{}) error {
	...
	requested := make([]map[string]string, 0, len(sessions))
	for key := range sessions {
		requested = append(requested, map[string]string{"source": key.source, "session": key.runID})
	}
	payload, err := json.Marshal(requested)
	if err != nil {
		return fmt.Errorf("encode published session targets: %w", err)
	}
	rows, err := transaction.QueryContext(ctx, `WITH requested AS (
  SELECT json_extract(value, '$.source') AS source, json_extract(value, '$.session') AS session_id
  FROM json_each(?)
)
SELECT requested.source, COALESCE(m.root_session_id, requested.session_id)
FROM requested
LEFT JOIN session_memberships m ON m.source = requested.source AND m.session_id = requested.session_id`, string(payload))
	if err != nil {
		return fmt.Errorf("query published session targets: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var sourceID, sessionID string
		if err := rows.Scan(&sourceID, &sessionID); err != nil {
			return fmt.Errorf("scan published session target: %w", err)
		}
		targets.Add(query.SessionTarget(sourceID, sessionID))
	}
	if err := rows.Err(); err != nil {
		return fmt.Errorf("iterate published session targets: %w", err)
	}
	return nil
}
...
func rollupCanApplyIncrementally(batch canonical.Batch, previous map[storedSpanKey]storedSpanScope) bool {
	for _, span := range batch.Spans {
		...
		if old.activityKind != string(span.Kind) || old.source != normalizeSource(span.Source) || old.session != span.Agent.RunID ||
			normalizedAgentID(old.agentID, old.session) != normalizedAgentID(span.Agent.AgentID, span.Agent.RunID) ||
...
func projectionTargets(batch canonical.Batch) []query.ChangeTarget {
	...
	for _, link := range batch.SessionLinks {
		set.Add(query.OverviewTarget())
		set.Add(query.SourceTarget(normalizeSource(link.Source)))
		set.Add(query.AllSessionsTarget())
		if link.ParentSessionID != "" {
			set.Add(query.SessionTarget(normalizeSource(link.Source), link.ParentSessionID))
		}
		if link.ChildSessionID != "" {
			set.Add(query.SessionTarget(normalizeSource(link.Source), link.ChildSessionID))
		}
	}
