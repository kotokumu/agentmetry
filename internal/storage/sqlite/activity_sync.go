...
func (store *Store) syncActivities(ctx context.Context, scopeKind, source, scopeID string, filter query.ActivitySyncFilter) (query.ActivitySyncPage, error) {
	...
	var sessionGraphSnapshot sessionGraph
	sessionMembers := map[string]struct{}{scopeID: {}}
	if scopeKind == "session" {
		ref := sessionRef{sourceID: source, sessionID: scopeID}
		sessionGraphSnapshot, err = loadSessionGroupWithReader(ctx, transaction, ref)
		if err != nil {
			return query.ActivitySyncPage{}, err
		}
		sessionMembers = make(map[string]struct{})
		for _, member := range sessionGraphSnapshot.members(sessionGraphSnapshot.root(ref)) {
			sessionMembers[member.sessionID] = struct{}{}
		}
	}
	...
	scopePredicate := "scope_id = ?"
	scopeArgs := []any{scopeID}
	if scopeKind == "session" {
		memberIDs := make([]string, 0, len(sessionMembers))
		for memberID := range sessionMembers {
			memberIDs = append(memberIDs, memberID)
		}
		payload, encodeErr := json.Marshal(memberIDs)
		if encodeErr != nil {
			return query.ActivitySyncPage{}, fmt.Errorf("encode session activity sync members: %w", encodeErr)
		}
		scopePredicate = "scope_id IN (SELECT value FROM json_each(?))"
		scopeArgs = []any{string(payload)}
	}
  ...
  WHERE scope_kind = ? AND source = ? AND %s AND sequence > ? AND sequence <= ?
)
SELECT activity_id, operation FROM ranked WHERE rank = 1
ORDER BY sequence ASC, id ASC LIMIT ? OFFSET ?`, scopePredicate)
	args := []any{scopeKind, source}
	args = append(args, scopeArgs...)
	args = append(args, filter.After.Sequence, through.Sequence, pageSize+1, offset)
	...
	for index := range result.Mutations {
		...
		if !exists || !activityBelongsToScope(activity, scopeKind, source, scopeID, sessionMembers) {
			mutation.Operation = query.ActivityMutationRemove
			continue
		}
		if scopeKind == "session" {
			activity = sessionGraphSnapshot.normalizeActivityAgent(activity)
		}
...
func activityBelongsToScope(activity query.Activity, scopeKind, source, scopeID string, sessionMembers map[string]struct{}) bool {
	if scopeKind == "session" {
		_, member := sessionMembers[activity.RunID]
		return activity.Source == source && member
	}
