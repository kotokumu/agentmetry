...
SELECT source, run_id, CASE WHEN agent_id = '' THEN 'main' ELSE agent_id END,
...
) GROUP BY source, run_id, CASE WHEN agent_id = '' THEN 'main' ELSE agent_id END`
}
...
WHERE source = ? AND run_id = ? AND agent_id = ?`, old.input, old.output, old.cacheRead, old.cacheWrite, old.reasoning, old.source, old.session, normalizedAgentID(old.agentID))
		if err != nil {
...
func normalizedAgentID(value string) string {
	if value == "" {
		return "main"
	}
	return value
